...
m_dg = 0.85;              % Drive Gear Mass
m_pg = 1.066;               % Driven Gear Mass
F_gear = 6.5359;      %force transmitted by gears
GR = 1.1; %gear ratio
...
L1 = (37/2)/1000; %distance from bearing to gear (37/2)

M1 = L1*F_b1; %moment at gear

T1 = 12.0803/GR; %torque at gear (speed reducer)

A = sqrt(4*((Kf*M1)^2)+3*(Kfs*T1)^2); %at gear

B = 0; %assumed no mean stresses

dS1 = 1000*((16*n/pi)*((A/(S_e*10^6))+(B/(S_u*10^6))))^(1/3);
...
L1 = (37/2)/1000; %distance from bearing 1 to gear (37/2)
L2 = (37/2)/1000; %distance from gear to bearing 2 (37/2)
L3 = (40.113 + 5.888)/(2*1000); %distance from bearing 2 to flywheel (40.113 + 5.888)/2
L4 = L3; %distance from flywheel to bearing 3 same
...
dS2_1 = 1000*((16*n/pi)*((A1/(S_e*10^6))+(B/(S_u*10^6))))^(1/3);
dS2_2 = 1000*((16*n/pi)*((A2/(S_e*10^6))+(B/(S_u*10^6))))^(1/3);
dS2_3 = 1000*((16*n/pi)*((A3/(S_e*10^6))+(B/(S_u*10^6))))^(1/3);
...
LB = (32.694 +61.994)/(2*1000);             %distance between two ball bearings (32.694 +61.994)/2 mm
LS = LB/2;          %distance between center of bearing and applied spring force
LK = 14.525/1000;             %distance from center of ball bearing to edge of crank (14.525 mm)
...
dS3_1 = 1000*((16*n/pi)*((A1/(S_e*10^6))+(B1/(S_u*10^6))))^(1/3); %Normal operation at edge of crank
dS3_2 = 1000*((16*n/pi)*((A2/(S_e*10^6))+(B1/(S_u*10^6))))^(1/3); %Worst case scenario at edge of crank
dS3_3 = 1000*((16*n/pi)*((A3/(S_e*10^6))+(B2/(S_u*10^6))))^(1/3); %Normal operation at center of crank
dS3_4 = 1000*((16*n/pi)*((A4/(S_e*10^6))+(B2/(S_u*10^6))))^(1/3); %worst case scenario at center of crank
